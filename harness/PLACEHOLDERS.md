...
| `COMPRESS_TO_KB` | init | The state file's post-action ceiling in bytes. Half of the post-action ceilings: it is what makes a punctuation rewrite pointless. |
| `COMPRESS_TO_SENTENCES` | init | The state file's post-action ceiling in sentences. The other half: it is what makes shaving words pointless. A landing satisfies both. |
| `WARN_KB` | init | The state file's soft cap, in bytes. Leave many sessions of growth above where the post-action ceilings lands — the band IS the debounce. |
